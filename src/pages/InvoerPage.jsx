...
export default function InvoerPage() {
    ...
    return (
      <div className="animate-scale-in">
        <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 text-center">
          <div className="w-12 h-12 mx-auto mb-4 border-3 border-primary-light border-t-primary rounded-full animate-spin" />
          <p className="text-base font-semibold text-[#1a1a1a]">Foto analyseren...</p>
          <p className="text-xs text-muted mt-1">Even geduld, AI herkent ingrediënten</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="animate-scale-in">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 text-center">
          ...
          <p className="text-sm font-semibold text-[#1a1a1a] mb-1">{aiError}</p>
      ...
      <div className="fixed inset-0 z-40" onClick={handleAiCancel}>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 animate-scale-in max-h-[80vh] overflow-y-auto">
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs text-muted">AI-herkenning</p>
            <button onClick={handleAiCancel} className="text-muted hover:text-[#1a1a1a] transition-colors">
          ...
          <input
            ...
            className="w-full px-4 py-3 bg-surface border border-gray-200 rounded-xl text-sm font-semibold text-[#1a1a1a] mb-4 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all"
          />
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
                    checked
                      ...
                      : 'bg-surface text-muted border border-gray-200 line-through'
                  }`}
                >
          ...
          <button
            ...
            className="w-full flex items-center justify-between px-4 py-3 mb-4 bg-surface rounded-xl"
          >
            <span className="text-sm text-[#1a1a1a]">Ook als maaltijd bewaren</span>
            <div className={`w-10 h-6 rounded-full transition-all duration-200 flex items-center ${
              saveAsTemplate ? 'bg-primary justify-end' : 'bg-muted-light justify-start'
            }`}>
    ...
    return (
      <div className="fixed inset-0 z-40" onClick={handleCancelMeal}>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 animate-scale-in">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-base font-semibold text-[#1a1a1a]">{selectedMeal.name}</p>
              <p className="text-xs text-muted mt-0.5">Vink ingrediënten aan of uit</p>
            </div>
            <button onClick={handleCancelMeal} className="text-muted hover:text-[#1a1a1a] transition-colors">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
  ...
  if (symptomPrompt) {
    return (
      <div className="animate-scale-in">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 text-center">
          <p className="text-base font-semibold text-[#1a1a1a] mb-1">Hoe voelt je maag?</p>
          <p className="text-xs text-muted mb-5">Tik op een niveau of sla over</p>
          ...
          <button
            onClick={handleSkip}
            className="text-sm text-muted hover:text-[#1a1a1a] transition-colors"
          >
            Overslaan
          </button>
        </div>
      ...
      <div className="flex bg-surface rounded-2xl p-1 mb-5">
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${
            mode === 'maaltijd' ? 'bg-white text-primary shadow-sm' : 'text-muted'
          }`}
        >
          Eten
        </button>
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${
            mode === 'klacht' ? 'bg-white text-red-500 shadow-sm' : 'text-muted'
          }`}
        >
          Klacht
        </button>
        ...
        <div className="animate-fade-in">
          ...
          {meals.length === 0 ? (
            ...
            <div className="space-y-2">
              ...
              <Link
                to="/maaltijden"
                className="block text-center text-xs text-muted hover:text-primary-dark py-3 transition-colors"
              >
                Maaltijden beheren
              </Link>
            </div>
...
function SymptomOnlyInput({ onSaved }) {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <p className="text-sm text-muted text-center">Hoe voelt je maag op dit moment?</p>
      ...
      <input
        ...
        className="w-full px-4 py-3.5 bg-white border border-gray-200 rounded-2xl text-base shadow-sm focus:outline-none focus:ring-2 focus:ring-red-300/50 focus:border-red-200 transition-all"
      />
