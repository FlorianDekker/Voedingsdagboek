...
export default function EntryCard({ entry }) {
  ...
  const barColor = isMeal ? '#65B741' : SEVERITY_COLORS[entry.severity]
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex overflow-hidden active:scale-[0.99] transition-all duration-150">
      ...
      <div className="flex-1 min-w-0 px-4 py-3.5">
        {isMeal ? (
          <>
            {entry.note && (
              <p className="text-[13px] font-semibold text-[#1a1a1a] mb-1.5">{entry.note}</p>
            )}
            <div className="flex flex-wrap gap-1">
              {entry.description.split(', ').map((ing, i) => (
                <span key={i} className="text-[11px] bg-primary-light text-primary-dark px-2 py-0.5 rounded-full capitalize font-medium">
                  {ing}
                </span>
              ))}
            </div>
          ...
          <>
            ...
            <span className="text-[10px] text-muted font-medium mt-2 block">{formatTime(entry.timestamp)}</span>
          </>
        )}
      </div>
      ...
      <button
        ...
        className="px-3 text-muted-light hover:text-red-400 active:text-red-500 transition-colors self-center"
      >
